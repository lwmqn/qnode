...
MqttNode.prototype._writeResrc = function (chkaccess, oid, iid, rid, value, callback) {
    var self = this,
        ...
        trg = this._target(oid, iid, rid),
        resrc = trg.value,
        acnt = mutils.getAccessCntl(oid, rid),
        writable = true;
...
MqttNode.prototype.enableReport = function (oid, iid, rid) {
    ...
    if (trg.type === TTYPE.obj) {
        ...
        notify = { oid: mutils.oidNumber(oid), data: null };
    } else if (trg.type === TTYPE.inst) {
        rpid = okey + ':' + iid;
        dumper = function (cb) {
            self._dumpObj(oid, iid, cb);
        };
        notify = { oid: mutils.oidNumber(oid), iid: iid, data: null };
    } else if (trg.type === TTYPE.rsc) {
        rkey = mutils.ridKey(oid, rid);
        rpid = okey + ':' + iid + ':' + rkey;
        dumper = function (cb) {
        ...
        notify = { oid: mutils.oidNumber(oid), iid: iid, rid: mutils.ridNumber(rid), data: null };
    }
