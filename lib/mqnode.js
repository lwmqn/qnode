...
MqttNode.prototype.connect = function (brokerUrl, opts) {
    ...
    lsnEvtsToRemove.forEach(function (evt) {
        _removePrivateListeners(this.mc, evt);
    });
...
MqttNode.prototype.setDevAttrs = function (attrs) {
    ...
    _.forEach(attrs, function (attr, key) {
        if (_.isNumber(attr.lifetime) && attr.lifetime !== this.lifetime) {
            this.lifetime = this.so.lwm2mServer.lifetime = update.lifetime = attr.lifetime;
            _lfUpdate(self, true);
        }
        if (_.isString(attr.ip) && attr.ip !== this.ip)
            this.ip = this.so.connMonitor.ip = update.ip = attr.ip;
