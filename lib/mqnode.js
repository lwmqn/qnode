...
const CMD = { read: 0, write: 1, discover: 2, writeAttrs: 3, execute: 4, observe: 5, notify: 6 };
const TAG = { notfound: '_notfound_', unreadable: '_unreadable_', exec: '_exec_', unwritable: '_unwritable_' };
const ERR = { success: 0, notfound: 1, unreadable: 2, unwritable: 3, unexecutable: 4, timeout: 5, noclient: 6 };
const RSP = { ok: 200, created: 201, deleted: 202, changed: 204, content: 205, badreq: 400,
...
MqttNode.prototype._init = function () {
    ...
    this.on('raw', function () {
        process.nextTick(function (topic, message, packet) {
            self._rawHdlr(topic, message, packet);
        });
    });
    ...
    this.on('_connected', function () {
        setTimeout(function () {
            // subscribe LWMQN interfaces and register to Shepherd
            self._lwmqnSubAndReg();
        }, 600);
    });
};
...
MqttNode.prototype._pubReq = function (intf, data, callback) {
    ...
    // [TODO] rsp callback signature? -> (err, rsp)
