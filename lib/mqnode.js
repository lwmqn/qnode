...
/*** Public Methods                                                                            ***/
/*************************************************************************************************/
function maybeCallback(cb, emitter) {
    return _.isFunction(cb) ? cb : function (err) {
...
MqttNode.prototype.connect = function (brokerUrl, opts, callback) {
    ...
    this.once('connect_cb', function (data) {
        callback(data.err, data.rsp);
        self.emit('login', data.rsp);
    });
