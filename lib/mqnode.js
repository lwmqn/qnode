        ...
        if (_.isNil(obj)) {
            ...
            _.forEach(obj, function (r, rid) {
                self.readResrc(oid, iid, rid, function (err, val) {
                    dump[mutils.ridNumber(oid, rid)] = val;
                    count -= 1;
                    if (count === 0 && _.isFunction(callback))
                        callback(null, dump);
                });
...
MqttNode.prototype.setDevAttrs = function (attrs) {
    var self = this,
        update = {};

    _.forEach(attrs, function (attr, key) {
        if (_.isNumber(attr.lifetime) && attr.lifetime !== this.lifetime) {
            this.lifetime = this.so.lwm2mServer[0].lifetime = update.lifetime = attr.lifetime;
            _lfUpdate(self, true);
        }
        if (_.isString(attr.ip) && attr.ip !== this.ip)
            this.ip = this.so.connMonitor[0].ip = update.ip = attr.ip;

        if (_.isString(attr.version) && attr.version !== this.version)
            this.version = update.version = attr.version;
    });
...
MqttNode.prototype.readResrc = function (oid, iid, rid, callback) {
    return _readResrc(this, true, oid, iid, rid, callback);
};
...
MqttNode.prototype.subscribe = function (topics, opts, callback) {
    ...
    return this.mc.subscribe(topics, opts, callback);   // function (err, granted)
};

MqttNode.prototype.unsubscribe = function (topics, callback) {
    return this.mc.unsubscribe(topics, callback);
};
...
MqttNode.prototype.pubRegister = function (callback) {
    ...
    _.forEach(this.so, function (obj, oid) {
        _.forEach(obj, function (inst, iid) {
            ...
            objList.push({ oid: mutils.oidNumber(oid), iid: iidNum });
        });
    });
...
MqttNode.prototype.pubDeregister = function (callback) {
    return _pubReq(this, 'deregister', { data: null }, callback);
};
...
function _reqTimeout(qn, key, delay) {
    qn._tobjs[key] = setTimeout(function () {
        qn.emit(key, null, { status: ERR.timeout });
...
function _readResrc(qn, chk, oid, iid, rid, callback) {
    ...
    function invokeCb(err, data, cb) {
        ...
        if (chk && !_.isNil(data)) _checkAndReportResrc(qn, oid, iid, rid, data);
    }
...
function _reqHdlr(qn, msg) {
    ...
    var trg = qn._target(msg.oid, msg.iid, msg.rid);
