...
SmartObject.prototype.resrcList = function (oid) {
    ...
    if (obj) {
        _.forOwn(obj, function (resrcs, iid) {
            resrcList[iid] = resrcList[iid] || [];
            _.forOwn(resrcs, function (val, rid) {
                if (!_.includes(excludeKeys, rid))
                    resrcList[iid].push(mutils.ridNumber(oid, rid));
            });
        });
    }
...
SmartObject.prototype.notify = function (oid, iid, rid, value, callback) {
    var iobj = this.findIObject(oid, iid),
        resrc,
        notifyData = {
            oid: undefined,
            iid: undefined,
    ...
    if (arguments.length === 4) {
        ...
        resrc = iobj[mutils.ridKey(oid, rid)];
        if (_.isObject(resrc) && _.isFunction(resrc.exec)) {
            if (value !== '_exec_')
                return;
        }
        ...
        _.forEach(value, function (n, r) {
            resrc = iobj[mutils.ridKey(oid, r)];
            if (_.isObject(resrc) && _.isFunction(resrc.exec)) {
                if (value !== '_exec_')
                    delete value[r];
            }
    ...
    if (!_.isUndefined(oid))
        notifyData.oid = mutils.oidNumber(oid);

    if (!_.isUndefined(iid))
        notifyData.iid = iid;

    if (!_.isUndefined(rid))
        notifyData.rid = mutils.ridNumber(oid, rid);
    ...
    return this.node.pubNotify(notifyData, callback);
};
...
var commonResrcOfOid = {
    'lwm2mServer': [ 'shortServerId', 'lifetime', 'defaultMinPeriod', 'defaultMaxPeriod', 'regUpdateTrigger' ],
