...
function _serverResponseHandler(qnode, intf, msg) {
    // rsp_data = { transId, status, data }
    var transId = msg.transId,
        prom = qnode._rspsToResolve[intf];
    
    prom = prom ? prom[transId] : undefined;

    if (!prom)
        return;

    clearTimeout(prom.tmoutCtrl);
    prom.deferred.resolve(msg);

    prom = null;
    qnode._rspsToResolve[intf][transId] = null;
    delete qnode._rspsToResolve[intf][transId];

    if (_.isEmpty(qnode._rspsToResolve[intf]))
        delete qnode._rspsToResolve[intf];
}
...
MqttNode.prototype._deferIntfPromise = function (intf, transId, prom) {
    var self = this;

    this._rspsToResolve[intf] = this._rspsToResolve[intf] || {};
    this._rspsToResolve[intf][transId] = {
        deferred: prom,
        tmoutCtrl: setTimeout(function () {         // pass 'inner timeout handling' to _clientResponseHandler
                _serverResponseHandler(self, intf, {
                    transId: transId,
                    status: 408,
                    data: null
                });
        ...
        network.get_active_interface(function(err, info) {
            if (err) {
                ...
                self.publish(regPubChannel, regData).done(function () {
                    self._deferIntfPromise('register', regData.transId, deferred);
                }, function(err) {
                    deferred.reject(err);
                });
            }
...
MqttNode.prototype.pubDeregister = function (callback) {
    ...
    this.publish(deregPubChannel, deregData).done(function () {
        self._deferIntfPromise('deregister', deregData.transId, deferred);
    }, function(err) {
        deferred.reject(err);
    });
...
MqttNode.prototype.pubUpdate = function (devAttrs, callback) {
    ...
    if (_.has(devAttrs, 'mac')) {
        ...
        devAttrs.transId = this.nextTransId();
        this.publish(updatePubChannel, devAttrs).done(function () {
            self._deferIntfPromise('update', devAttrs.transId, deferred);
        }, function(err) {
            deferred.reject(err);
        });
    }
...
MqttNode.prototype.pubNotify = function (data, callback) {
    ...
    data.transId = this.nextTransId();
    this.publish(notifyPubChannel, data).done(function () {
        self._deferIntfPromise('notify', data.transId, deferred);
    }, function(err) {
        deferred.reject(err);
    });
...
MqttNode.prototype.pingServer = function (callback) {
    ...
    this.publish(pingPubChannel, pingData).done(function () {
        self._deferIntfPromise('notify', pingData.transId, deferred);
    }, function(err) {
        deferred.reject(err);
    });
