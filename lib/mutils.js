...
mutils.oidKey = function (oid) {
    var oidItem = parseInt(oid);

    if (_.isNaN(oidItem))
        return oid;

    oidItem = lwm2mid.getOid(oid);
...
mutils.ridKey = function (oid, rid) {
    ...
    var ridItem = parseInt(rid);
    if (_.isNaN(ridItem))
        return rid;

    ridItem = lwm2mid.getRid(ooid, orid);
...
mutils.getAccessCntl = function (oid, rid) {
    var chr = lwm2mid.getSpecificResrcChar(oid, rid);
    if (chr)
        return chr.access;
    else
        return;
