...
const debug = require('debug')('lwmqn-qnode:msgHdlr')
const warn = require('debug')('lwmqn-qnode:msgHdlr:warn')
...
module.exports = {
  _rawHdlr (qn, topic, message, packet) {
    ...
    qn.decrypt(message, (err, decrypted) => {
      ...
      if (err) {
        warn('decryption fails') // log 'decryption fails'
        return
      }
      ...
      const strmsg = (message instanceof Buffer) ? message.toString() : message
  ...
  _reqHdlr (qn, msg) {
    ...
    switch (rsp.cmdId) {
      case CMD.read:
        reqMsgHdlr = this._readReqHandler
        break
      case CMD.write:
        reqMsgHdlr = this._writeReqHandler
        break
      case CMD.writeAttrs:
        reqMsgHdlr = this._writeAttrsReqHandler
        break
      case CMD.discover:
        reqMsgHdlr = this._discoverReqHandler
        break
      case CMD.execute:
        reqMsgHdlr = this._executeReqHandler
        break
      case CMD.observe:
        reqMsgHdlr = this._observeReqHandler
        break
      case CMD.ping:
        reqMsgHdlr = this._pingReqHandler
        break
      case CMD.identify:
        reqMsgHdlr = this._identifyReqHandler
        break
      default:
        reqMsgHdlr = this._unknownReqHandler
    }
  ...
  _writeAttrsReqHandler (qn, trg, msg, cb) {
    ...
    const attrs = _.cloneDeep(reporter.getAttrs(qn, msg.oid, msg.iid, msg.rid))
