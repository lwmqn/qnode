            ...
            case qn._subics.register:
                if (_.isObject(jmsg)) {
                    ...
                    if (jmsg.status == RSP.ok || jmsg.status == RSP.created)
                        helper._lfUpdate(qn, true);
                    else
                        helper._lfUpdate(qn, false);
                }
                break;
...
handlers._readReqHandler = function(qn, trg, msg, cb) {
    ...
    if (trg.type === TTYPE.obj) {
        qn._dumpObj(msg.oid, function (err, data) {
            cb(status, data);
        });
    } else if (trg.type === TTYPE.inst) {
        qn._dumpObj(msg.oid, msg.iid, function (err, data) {
            cb(status, data);
        });
...
handlers._writeAttrsReqHandler = function (qn, trg, msg, cb) {
    ...
    if (trg.type === TTYPE.obj) {
        ...
        qn._setAttrs(msg.oid, msg.data);
    } else if (trg.type === TTYPE.inst) {
        if (msg.data.cancel)
            qn.disableReport(msg.oid, msg.iid);

        qn._setAttrs(msg.oid, msg.iid, msg.data);
    } else if (trg.type === TTYPE.rsc) {
        if (msg.data.cancel)
            qn.disableReport(msg.oid, msg.iid, msg.rid);

        qn._setAttrs(msg.oid, msg.iid, msg.rid, msg.data);
    }

    attrs = _.cloneDeep(qn._getAttrs(msg.oid, msg.iid, msg.rid));
...
handlers._discoverReqHandler = function (qn, trg, msg, cb) {
    ...
    if (trg.type === TTYPE.obj) {
        attrs = _.cloneDeep(qn._getAttrs(msg.oid));
        ...
        attrs = _.cloneDeep(qn._getAttrs(msg.oid, msg.iid));
    } else if (trg.type === TTYPE.rsc) {
        attrs = _.cloneDeep(qn._getAttrs(msg.oid, msg.iid, msg.rid));
    }
