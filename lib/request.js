...
const debug = require('debug')('lwmqn-qnode:request')
...
function _req (qn, intf, data, callback) {
  let evt
  let err
  ...
  if (!qn.mc || !qn._connected) {
    ...
    _reqTimeout(qn, evt, CNST.REQ_TIMEOUT)
...
module.exports = {
  register (qn, callback) {
    ...
    return _req(qn, 'register', data, (err, rsp) => {
  ...
  deregister (qn, callback) {
    return _req(qn, 'deregister', { transId: null }, (err, rsp) => {
      if (!err) {
  ...
  checkin: (qn, callback) => _req(qn, 'schedule', { transId: null, sleep: false }, callback),
  checkout: (qn, duration, callback) => _req(qn, 'schedule', { transId: null, sleep: true, duration }, callback),
  update: (qn, devAttrs, callback) => _req(qn, 'update', _.omit(devAttrs, ['mac', 'clientId']), callback), // Change of mac address and clientId at runtime will be ignored
  notify (qn, data, callback) {
    return _req(qn, 'notify', data, (err, rsp) => {
  ...
  ping (qn, callback) {
    ...
    return _req(qn, 'ping', data, (err, rsp) => {
