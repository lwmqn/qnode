...
function IObject(oid, ridSets) {    // prefer [ { rid: rvalue }, { rid: rvalue  } ]
    var self = this;

    this.oid = mutils.oidKey(oid);
    this.iid = null;
    this.attrs = undefined;         // attrs are used for observation
    this.so = null;                 // points to smart object who own this iObject
...
IObject.prototype.readResrc = function (rid, callback) {
    ...
    if (_.isUndefined(resrc)) {
        ...
        deferred.resolve(resrc);                            // [FIXME] resrc only supports values if it is an array
    }
...
IObject.prototype.findAttrs = function () {
    if (_.isUndefined(this.attrs))
        return this.so ? this.so.attrs : undefined;
    else
        return this.attrs;
};
...
IObject.prototype.findResrcAttrs = function (rid) {
    ...
    if (_.isUndefined(resrcAttrs))
        resrcAttrs = this.resrcAttrs[ridKey] = this.attrs;

    if (_.isUndefined(resrcAttrs))
        resrcAttrs = this.resrcAttrs[ridKey] = this.so.attrs;
