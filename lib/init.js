...
module.exports = function (qn) {
  ...
  qn.on('_request', (err, msg) => { // emit @ msgHandlers._rawHdlr with (err, jmsg)
...
function _lwmqnSubAndReg (qn) {
  const subics1 = [qn._subics.register, qn._subics.deregister, qn._subics.request]
  const subics2 = [qn._subics.notify, qn._subics.update, qn._subics.ping, qn._subics.announce, qn._subics.schedule]
  let regRsp

  function sub2Callback (err, granted) {
    if (err) {
      qn.emit('_connect_cb', { err, rsp: null })
