...
reporter.enableReport = function (qn, oid, iid, rid) {
    var trg = qn._target(oid, iid, rid),
        rAttrs = reporter.getAttrs(qn, oid, iid, rid),
        okey = mutils.oidKey(oid),
        rkey, rpid, pmin, pmax, rRpt, dumper, notify;
    ...
    //         qn._readResrc(oid, iid, rid);    // just read it, reporter._checkAndReportResrc() will be invoked
    //     }, rAttrs.pintvl || 500);
    // }
...
reporter.getAttrs = function (qn, oid, iid, rid) {
    ...
    defaultAttrs = {
        pmin: qn.so.get('lwm2mServer', 0, 'defaultMinPeriod'),
        pmax: qn.so.get('lwm2mServer', 0, 'defaultMaxPeriod'),
        mute: true,
        cancel: true,
        lastRpVal: null
    };
...
reporter.checkAndReportResource = function (qn, oid, iid, rid, currVal) {
    var rAttrs = reporter.getAttrs(qn, oid, iid, rid),
        rpt = false,
        gt, lt, step, lastrp;
