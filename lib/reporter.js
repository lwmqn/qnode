...
module.exports = {
  enableReport (qn, oid, iid, rid) {
    const trg = qn._target(oid, iid, rid) // { type, exist }
    const so = qn.getSmartObject()
    const rAttrs = this.getAttrs(qn, oid, iid, rid) // { pmin, pmax, mute, cancel, lastRpVal }
    ...
    if (trg.type === TTYPE.obj) {
      rpid = okey
      dumper = (cb) => (so.dump(oid, { restrict: true }, cb))
      notifier = { oid: mutils.oidNum(oid), data: null }
    } else if (trg.type === TTYPE.inst) {
      rpid = `${okey}:${iid}`
      dumper = (cb) => (so.dump(oid, iid, { restrict: true }, cb))
      notifier = { oid: mutils.oidNum(oid), iid, data: null }
    } else if (trg.type === TTYPE.rsc) {
      rkey = mutils.ridKey(oid, rid)
      rpid = `${okey}:${iid}:${rkey}`
      dumper = (cb) => (so.read(oid, iid, rid, { restrict: true }, cb))
      notifier = { oid: mutils.oidNum(oid), iid, rid: mutils.ridNum(rid), data: null }
    } else {
      return false
    }

    const pmin = rAttrs.pmin * 1000
    const pmax = rAttrs.pmax * 1000
    rAttrs.cancel = false
    rAttrs.mute = true
    qn._reporters[rpid] = { min: null, max: null, poller: null }
    const rRpt = qn._reporters[rpid]
  ...
  clear (qn) {
    Object.entries(qn._repAttrs).forEach(([rpid, rAttrs]) => {
  ...
  setAttrs (qn, oid, iid, rid, attrs) {
    ...
    const trg = qn._target(oid, iid, rid)
  ...
  checkAndReportResource (qn, oid, iid, rid, currVal) {
    const rAttrs = this.getAttrs(qn, oid, iid, rid) // { pmin, pmax, mute, cancel, lastRpVal }
    let rpt = false
    ...
    const gt = rAttrs.gt
    const lt = rAttrs.lt
    const step = rAttrs.stp
    const lastrp = rAttrs.lastRpVal

    if (rAttrs.cancel || rAttrs.mute) return false

    if (_.isObject(currVal)) {
      if (_.isObject(rAttrs.lastRpVal)) {
        Object.entries(rAttrs.lastRpVal).forEach(([k, v]) => {
